...
{
    public class FSMDLogicArray<T> : FSMDBaseLogic
    {

        #region 初始化

        public void Set(T defaultLogic, params (T key, FSMDBaseLogic logic)[] logics)
        {
        ...
        private Dictionary<T, FSMDBaseLogic> logics = new Dictionary<T, FSMDBaseLogic>();

        public FSMDBaseLogic GetLogic(T key)
        {
            if (logics.ContainsKey(key))
            {
                return logics[key];
            }
            return default(FSMDBaseLogic);
        }
        ...
        public void AddLogics(params (T key, FSMDBaseLogic logic)[] logics)
        {
            foreach (var logic in logics)
            {
        ...
        public void ChangeLogic(T key, FSMDBaseLogic logic)
        {
            RemoveLogic(key);
            AddLogic(key, logic);
        }

        public void ChangeLogics(params (T key, FSMDBaseLogic logic)[] logics)
        {
            foreach (var logic in logics)
            {
        ...
        public void RemoveLogic(T key)
        {
            if (logics.ContainsKey(key))
            {
                FSMDManager.Instance.logics.Destory(logics[key].key);
                logics.Remove(key);
            }
        }
        ...
        public void RemoveLogic(FSMDBaseLogic logic)
        {
            if (logics.ContainsValue(logic))
            {
                foreach (var item in logics)
                {
                    if (item.Value == logic)
                    {
                        FSMDManager.Instance.states.Destory(logic.key);
        ...
        public void RemoveLogics(FSMDBaseLogic[] logics)
        {
        ...
        public void Clear()
        {
            foreach (var logic in logics)
            {
                FSMDManager.Instance.logics.Destory(logic.Value.key);
            }
            logics.Clear();
        }
        ...
        private void Update()
        {
            ...
            {
                if (logics[currentLogic].transition != null)
                {
                    T newState = (logics[currentLogic].transition as FSMDBaseTransition<T>).Change(context);
                    Change(newState);
                }
            }
        }
